...
//GET members in Neighborhood
	...
	test("Get members of neighborhood with Missing Authorization", function(assert) {
    	var done = assert.async();
    	TestUtils.sendRequestMissingAuthorization(Globals.baseURL + "rest/neighborhood/" + rootNhId + "/member", null, "GET").then(function(data){
			assert.notOk(isNullObject(data), "Member data shouldn't be null");
...
//Create member test
	...
	//    UserTests.createNewUser().then(function(result) {
      	//	user = result[0];
      		...
      		TestUtils.sendRequestMissingAuthorization(Globals.baseURL + "rest/neighborhood/" + rootNhId + "/member", membershipData, "POST").then(function(data){
