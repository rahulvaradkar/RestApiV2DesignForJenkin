var UserTests =(function( module, test ) {
	...
	function createNewUser() {
		...
		return TestUtils.sendRequest(Globals.baseURL + "rest/user",data, Globals.authorization, "POST");		
	  }
	...
	QUnit.test("Test Case For Single User", function( assert ) {
		...
		TestUtils.sendRequest(Globals.baseURL + "rest/user/1",null, Globals.authorization, "GET").then(function(result){
			var flag=false;
	...
	QUnit.test("Test Case For zero user Id", function( assert ) {
		var done = assert.async();		
		TestUtils.sendRequest(Globals.baseURL + "rest/user/0",null, Globals.authorization, "GET").then(function(result){			
			assert.ok(result != null , "Response Should not null")
	...
	QUnit.test("Test Case For User Not Found", function( assert ) {
		var done = assert.async();		
		TestUtils.sendRequest(Globals.baseURL + "rest/user/789689",null, Globals.authorization, "GET").then(function(result){			
			assert.ok(result != null , "Response Should not null")
	...
	QUnit.test( "Test Case For all Active Users", function( assert ) {
		...
		TestUtils.sendRequest(Globals.baseURL + "rest/user?active=true",null, Globals.authorization, "GET").then(function(result){			
			if (result.length > 0)
			{
				count_flag=true;
			}
	...
	QUnit.test("Test Case For all non-active Users", function( assert ) {
		...
		var flag= false;
		var count_flag=false;
		TestUtils.sendRequest(Globals.baseURL + "rest/user?active=false",null, Globals.authorization, "GET").then(function(result){
			if (result.length >= 0)
			{
				count_flag=true;
			}
...
<<<<<<< HEAD
	QUnit.test("Test Case For Posting Duplicate user", function( assert ) {
		...
		TestUtils.sendRequest(Globals.baseURL + "rest/user", data, Globals.authorization, "POST").then(function(result){			
			assert.ok(result != null , "Response Should not null")
...
>>>>>>> 258602749d5ee55b00ddf3ccf0b377566efc43bd
	QUnit.test("Test Case For Invalid Authorization for Posting new user", function( assert ) {
		...
		TestUtils.sendRequest(Globals.baseURL + "rest/user",data, Globals.invalidAuthorization, "POST").then(function(result){			
			assert.ok(result != null , "Response Should not null")
...
<<<<<<< HEAD
	QUnit.test("Put test cases for User", function(assert){
		...
		TestUtils.sendPutRequest(Globals.baseURL + "rest/user", data, Globals.authorization).then(function(res){		
			assert.equal(typeof res, "string", "response should be string !");
			assert.notEqual(res.length, 0, "should not be null !");
...
<<<<<<< HEAD
	QUnit.test( "Test case for Deleting User", function( assert ) {
		var done=assert.async();		
		TestUtils.sendDeleteRequest(Globals.baseURL + "rest/user/" + userid, Globals.authorization).then(function(res){
			var getResponse = res;
 			var checkSubString = getResponse.indexOf("de-activated successfully");
  			assert.ok((checkSubString > 0),"User Deleted Successfully!");
			assert.equal(typeof res, "string", "response should be string !");
...
>>>>>>> 258602749d5ee55b00ddf3ccf0b377566efc43bd
	...
	QUnit.test( "Test case for Deleting Non existing User ", function( assert ) {
		var done=assert.async();		
		TestUtils.sendRequest(Globals.baseURL + "rest/user/9999", null, Globals.authorization, "DELETE").then(function(result){			
			assert.ok(result != null , "Response Should not null")
	...
	QUnit.test( "Test case for Deleting User With Negative Userid ", function(assert) {
		var done=assert.async();		
		TestUtils.sendRequest(Globals.baseURL + "rest/user/-1", null, Globals.authorization, "DELETE").then(function(result){			
			assert.ok(result != null , "Response Should not null")
