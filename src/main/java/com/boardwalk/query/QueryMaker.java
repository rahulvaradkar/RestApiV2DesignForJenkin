	...
	*/
	...
	public static String getFiltredQueryStringForBrowser(String asRowQuery, boolean withbaseline, boolean asOfDate, String reqColids)
	{
		...
		lsRetStr.append(" UNION ALL ");
			...
			lsRetStr.append("	IF OBJECT_ID('tempdb..#CELL_TEMP') IS NOT NULL DROP TABLE #CELL_TEMP ");
		}
		else
		{
			if(asOfDate)
...
//					lsRetStr.append("	ORDER BY BWROW.SEQUENCE_NUMBER,  c.SEQUENCE_NUMBER ");
	...
	public static String getFiltredQueryBefXL(String asRowQuery)
	{
		...
		" IF(SELECT [DESCRIPTION] FROM BW_TXS WHERE TX_ID = ?) = 'Link export new table' " +
		" BEGIN " +
		" 	INSERT INTO #CTXS " +
		" 	SELECT  " +
...
// BW_GET_STATUS_CHANGES_FOR_TID
	...
	public static String getFiltredTransactionListBL(String asRowQuery)
	{
		...
		lsRetStr.append(" union");
		/* formula update */
		lsRetStr.append(" select bw_string_value.tx_id, bw_txs.created_on, bw_user.email_address as created_by, bw_txs.comment_ , action='FRMUPD'"); //Modified to fix the Issue Id: 12279(Show Excel dragged formulas also in the Table History)
		lsRetStr.append(" FROM #ACCESSIBLE_ROWS  AS bwrow,");
		lsRetStr.append(" bw_txs,");
		lsRetStr.append(" bw_string_value,");
		lsRetStr.append(" bw_strval_formula,");
		//lsRetStr.append(" bw_formula_value,"); //Modified to fix the Issue Id: 12279(Show Excel dragged formulas also in the Table History)
		lsRetStr.append(" bw_cell,");
		lsRetStr.append(" bw_user");
		...
		lsRetStr.append(" and bw_string_value.tx_id = bw_txs.tx_id"); //Modified to fix the Issue Id: 12279(Show Excel dragged formulas also in the Table History)
		lsRetStr.append(" and bw_txs.created_on >= ? "); //SDATE
		lsRetStr.append(" and bw_txs.created_on <= ? "); //EDATE
		lsRetStr.append(" and bw_txs.created_by = bw_user.id");
	...
	//Added by Lakshman on 20171108 to avoid self joins on BW_CELL for performance gain
	public static String getTransactionsForUserView(String asRowQuery)
	{
		String lsQueryStr = asRowQuery + 

		" SELECT	BW_TXS.TX_ID  " + 
		" INTO		#USER_TXS " + 
		" FROM		BW_TXS, BW_USER_EXPORT_TID  " + 
		" WHERE		BW_TXS.CREATED_BY				= ?  " + 
		" AND		BW_USER_EXPORT_TID.BW_USER_ID	= ? " + 
		" AND		BW_USER_EXPORT_TID.TX_ID		= BW_TXS.TX_ID " + 
		" AND		BW_USER_EXPORT_TID.BW_TBL_ID	= ? " + 
		" AND		BW_TXS.TX_ID					> ? " + 

		" SELECT	BC.ID, BC.BW_COLUMN_ID " + 
		" INTO		#USER_CELLS " + 
		" FROM		BW_CELL BC,BW_GETCOLUMNACCESSNEW(? , ? , ?) COL,#ACCESSIBLE_ROWS ROW " + 
		" WHERE		BC.BW_COLUMN_ID = COL.COLID  " + 
		" AND		BC.BW_ROW_ID	= ROW.ROWID " + 

		" SELECT TX_ID  " + 
		" FROM( " + 
		" 	SELECT	MAX(U.TX_ID) AS TX_ID " + 
		" 	FROM( " + 
		" 		SELECT  MAX(BW_STRING_VALUE.TX_ID) AS TX_ID  " + 
		" 		FROM	BW_STRING_VALUE,#USER_CELLS BW_CELL,#USER_TXS BW_TXS " + 
		" 		WHERE	BW_TXS.TX_ID				= BW_STRING_VALUE.TX_ID " + 
		" 		AND		BW_STRING_VALUE.BW_CELL_ID	= BW_CELL.ID  " + 
		" 		UNION	ALL " + 
		" 		SELECT  MAX(BW_CELL_STATUS.TX_ID) AS TX_ID " + 
		" 		FROM	BW_CELL_STATUS,#USER_CELLS BW_CELL,#USER_TXS BW_TXS " + 
		" 		WHERE	BW_TXS.TX_ID				= BW_CELL_STATUS.TX_ID  " + 
		" 		AND		BW_CELL_STATUS.BW_CELL_ID	= BW_CELL.ID  " + 
		" 	)	AS U  " + 
		" )	AS A  " + 
