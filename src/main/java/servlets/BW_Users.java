...
public class BW_Users extends BWServlet   implements
 SingleThreadModel
{
    ServletContext sc;
	...
	public static boolean IsCreatorOfCollab(int userId)
	{
		 Connection connection = null;
		 int countMemberId = -1;
		 boolean retValue = false;
		PreparedStatement stmt = null;
		   try {
				...
				System.out.println("value of userId^^^^^^^^^"+userId);
				 String query = "SELECT COUNT(BW_COLLAB.MEMBER_ID) COUNT FROM BW_COLLAB,BW_MEMBER WHERE BW_COLLAB.MEMBER_ID = BW_MEMBER.ID AND BW_MEMBER.USER_ID =? ";
				 stmt = connection.prepareStatement(query);
