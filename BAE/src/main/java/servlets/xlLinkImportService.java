...
public class xlLinkImportService extends xlService implements SingleThreadModel
{
		...
		HttpServletResponse response)throws ServletException, IOException
	{
		...
		try
		{
			...
			int tableId = Integer.parseInt(requestInfo[4]);
			...
			if( requestInfo.length == 9) //Modified by Tekvision on 20180207 for Clear Text Password(Issue Id: 14241)
			{
				...
				System.out.println("xlLinkImportService: nhHierarchy: " + nhHierarchy);
				System.out.println("xlLinkImportService: checkMembershipStatus: " + checkMembershipStatus);
				
				if( checkMembershipStatus <= 0 ){
					failureReason = "checkMembershipStatusFalse";
					System.out.println("Inside xlLinkImportService: Failure: " + failureReason);
					return;
				}
			}
...
//System.out.println Time to getTableBuffer
		...
		finally
		{
			...
			//Added to get the Membership ID in case of Multiple Membership
			else if(failureReason == "checkMembershipStatusFalse")
			{
