...
public class xlExportChangesService extends xlService implements SingleThreadModel
{
		...
		HttpServletResponse response)throws ServletException, IOException
	{
		...
		for (int ti = 0; ti < MAX_RETRY_ATTEMPTS; ti++)
		{
			try
			{
				...
				processHeader(connection, reader.getNextContent());
				...
				processRows(connection, tid, reader.getNextContent());

				String cellColBuffer = null;
				while ((cellColBuffer = reader.getNextContent()) != null )
				{
				...
				if (criteriaTableId > 0)
				{
					...
					String lsRowQuery  = TableViewManager.getRowQuery(connection, tableId, userId, criteriaTableId, true, "LATEST", "RESULTSET");
	...
	public void processHeader(Connection connection, String sub) throws BoardwalkException, SystemException, SQLException
	{
		getElapsedTime();
		...
		String lsRowQuery = TableViewManager.getRowQuery(connection, tableId, userId, criteriaTableId, true, view, "TABLE"); //Modified by Lakshman on 20171108 to avoid self joins on BW_CELL for performance gain
		...
		if (accessTableId > 0)
		{
			...
			colCellAccess = TableViewManager.getColumnAcccess(connection, tableId, accessTableId, userId);
			...
			while (columnConditionalAccessIter.hasNext())
			{
				...
				if (colCellAccess.get(colId) instanceof String){
					...
					if(matcher.matches())
					{
						...
						if (rowSet == null)
						{
							String rowQuery = TableViewManager.getRowQuery(connection, TableViewManager.getCriteriaForDynamicView(accessInstr), tableId, true, "RESULTSET");
