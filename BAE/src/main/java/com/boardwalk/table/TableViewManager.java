		...
		throws SystemException
	{
		...
		try
		{
			...
			while (ai.hasNext())
			{
				...
				String srcRowQuery = getRowQuery(connection, sourceTableId, 1, sourceFilterId, true, "LATEST", "RESULTSET"); //Modified by Lakshman on 20171108 to avoid self joins on BW_CELL for performance gain
				...
				String targetRowQuery = getRowQuery(connection, targetTableId, 1, targetFilterId, true, "LATEST", "RESULTSET"); //Modified by Lakshman on 20171108 to avoid self joins on BW_CELL for performance gain
...
//	}
	...
	{
		System.out.println("Inside TableViewManager.getRowQuery");
		...
		q_temp = q_temp + " SELECT BW_CELL.BW_ROW_ID,BW_CELL.BW_COLUMN_ID,BW_CELL.STRING_VALUE ";
		q_temp = q_temp + " INTO #CELL_TEMP FROM BW_CELL,BW_COLUMN ";
		q_temp = q_temp + " WHERE BW_COLUMN.ID = BW_CELL.BW_COLUMN_ID ";
		q_temp = q_temp + " AND BW_COLUMN.BW_TBL_ID = " + a_table_id + " ";
		...
		while (ri.hasNext())
		{
			...
			while (cri.hasNext())
			{
				...
				if (fc.gettrgtColumnId() > 0)
					q_from = q_from + "#CELL_TEMP AS CellCol" + fcColNum + " ";
				else
					q_from = q_from +
								  "BW_COLUMN AS Col" + fcColNum + " , " +
								  "#CELL_TEMP AS CellCol" + fcColNum + " ";

				q_where = q_where + " AND	CellCol" + fcColNum + ".BW_ROW_ID = BW_ROW.ID ";

				if (fc.gettrgtColumnId() > 0)
					q_where = q_where +	" AND	CellCol" + fcColNum + ".BW_COLUMN_ID = " + fc.gettrgtColumnId() + " ";
				else
					q_where = q_where + " AND	Col" + fcColNum + ".NAME = '" + BoardwalkUtility.replaceSQLString(fc.getColumnName()) + "'  " +
				...
				{
					q_where = q_where + " AND	CellCol" + fcColNum + ".STRING_VALUE IN " + fc.getValue();
				}
				else
					q_where = q_where + " AND	CellCol" + fcColNum + ".STRING_VALUE = '" + BoardwalkUtility.replaceSQLString(fc.getValue()) + "' " ;

				columnName = "'" + BoardwalkUtility.replaceSQLString(fc.getColumnName()) + "'";

				boolean isFound = concatColumns.contains(columnName);

				if (!isFound)
				{
					if (concatColumns == "")
						concatColumns = columnName;
					else
						concatColumns = concatColumns + "," + columnName;
				}
			...
			System.out.println("Inside TableViewManager.getRowQuery: Processing criterion = " + currCriterion);
			
			if (processedCriteria.get(currCriterion) != null)
			{
				System.out.println("Inside TableViewManager.getRowQuery: Criteria already processed. Skipping...");
			}
			else
			{
				if (query == null)
		...
		{
			if (resultType == "TABLE")
				query = " CREATE TABLE #ACCESSIBLE_ROWS (ROWID  INT  PRIMARY KEY NOT NULL) INSERT INTO #ACCESSIBLE_ROWS(ROWID) " + query;
			
			if (concatColumns.trim().length() > 0)
			{
				q_temp = q_temp + " AND BW_COLUMN.[NAME] IN ( " + concatColumns + " ) ";
	...
	throws SQLException
	{
		...
		query = getRowQuery(connection, criteria, a_table_id, active, resultType); //Modified by Lakshman on 20171108 to avoid self joins on BW_CELL for performance gain
