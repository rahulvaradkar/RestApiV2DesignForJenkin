...
import com.boardwalk.exception.BoardwalkException;
 ...
 public class BoardwalkColumnGroup
 implements Comparable
 {
     ...
     private BoardwalkColumnGroup(){}

     public BoardwalkColumnGroup ( String name,
     								  int startIndex,
     								  int endIndex,
							...
							String preAction, String postAction)
	{
	 ...
	 {
		 if (!(obj instanceof BoardwalkColumnGroup)) return false;
		 if (((BoardwalkColumnGroup)obj).getStartIndex() == _startIndex &&
		     ((BoardwalkColumnGroup)obj).getEndIndex() == _endIndex)
		 {
			 return true;
		 }
		 else
	 ...
	 {
		 BoardwalkColumnGroup bcg = (BoardwalkColumnGroup)o;
		 if (_startIndex < bcg.getStartIndex())
		 	return -1;
		 else if (_startIndex > bcg.getStartIndex())
		 	return 1;
		 else
		 	return 0;
	 }
