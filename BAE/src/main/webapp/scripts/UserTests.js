var UserTests =(function( module, test ) {
	...
	function createNewUser() {
		...
		return TestUtils.sendPostRequest(Globals.baseURL + "rest/v1/user",data);		
	  }
	...
	QUnit.test("Test Case For Single User", function( assert ) {
		...
		TestUtils.sendGetRequest(Globals.baseURL + "rest/v1/user/1").then(function(result){
			var flag=false;
	...
	QUnit.test("Test Case For User With id=2", function( assert ) {
		...
		TestUtils.sendGetRequest(Globals.baseURL + "rest/v1/user/2").then(function(result){
			var flag=false;
	...
	QUnit.test("Test Case For zero user Id", function( assert ) {
		var done = assert.async();		
		TestUtils.sendGetRequest(Globals.baseURL + "rest/v1/user/0").then(function(result){			
			assert.ok(result != null , "Response Should not null")
	...
	QUnit.test("Test Case For User Not Found", function( assert ) {
		var done = assert.async();		
		TestUtils.sendGetRequest(Globals.baseURL + "rest/v1/user/789689").then(function(result){			
			assert.ok(result != null , "Response Should not null")
	...
	QUnit.test("Test Case For Missing Authorization for user ID = 1", function( assert ) {
		var done = assert.async();	
		TestUtils.sendRequestMissingAuthorization(Globals.baseURL + "rest/v1/user/1", null, "GET").then(function(result){			
			assert.ok(result != null , "Response Should not null")
	...
	QUnit.test( "Test Case For all Active Users", function( assert ) {
		...
		TestUtils.sendGetRequest(Globals.baseURL + "rest/v1/user?active=true").then(function(result){			
			if (result.length > 0)
			{
				count_flag=true;
			}
	...
	QUnit.test("Test Case For all non-active Users", function( assert ) {
		...
		var flag= false;
		var count_flag=false;
		TestUtils.sendGetRequest(Globals.baseURL + "rest/v1/user?active=false").then(function(result){
			if (result.length >= 0)
			{
				count_flag=true;
			}
...
/*	
	...
	QUnit.test( "Test case for Deleting User", function( assert ) {
		...
		console.log(Globals.baseURL + "rest/v1/user/"+userid);		
		TestUtils.sendDeleteRequest(Globals.baseURL + "rest/v1/user/"+userid).then(function(res){
			var getResponse = res;
...
*/	
	QUnit.test("Test Case For Invalid Authorization for Posting new user", function( assert ) {
		...
		TestUtils.sendRequestInvalidAuthorization(Globals.baseURL + "rest/v1/user",data, "POST").then(function(result){			
			assert.ok(result != null , "Response Should not null")
	...
	QUnit.test( "Post test cases for new User with Blank Name And Password", function( assert ) {
		...
		TestUtils.sendPostRequest(Globals.baseURL + "rest/v1/user",data).then(function(result){
			var json = result[0];
			assert.notEqual(result.length,0,"object should not be empty !");
			if(Object.keys(json).length == 3)
			{  
	...
	QUnit.test("Put test cases for User with Blank email, firstName, lastName", function(assert){
		...
		TestUtils.sendPutRequest(Globals.baseURL + "rest/v1/user",data).then(function(res){
	...
	QUnit.test( "Test case for Deleting User with Missing Authorization", function( assert ) {
		var done=assert.async();		
		TestUtils.sendRequestMissingAuthorization(Globals.baseURL + "rest/v1/user/1", null, "DELETE").then(function(result){	
			console.log((result));	
