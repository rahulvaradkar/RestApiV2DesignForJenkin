(function( module, test ) {
    ...
    QUnit.test("Reading Whiteboard of Collaboration ID=1001", function(assert){
        ...
        TestUtils.sendGetRequest( Globals.baseURL + "rest/v1/collaboration/"+Globals.Collab_Id+"/whiteboard").then(function(result){
            if(result.length > 0)
            {	
                count_flag=true;
            }
    ...
    QUnit.test("Reading Whiteboard of not existed Collaboration ID=999", function(assert){
        var done = assert.async();
        TestUtils.sendGetRequest( Globals.baseURL + "rest/v1/collaboration/999/whiteboard").then(function(result){
 ...
 /*   
    QUnit.test( "Posting new Whiteboard", function( assert ) {
        ...
        TestUtils.sendPostRequest(Globals.baseURL + "rest/v1/collaboration/"+Globals.Collab_Id+"/whiteboard",data).then(function(result){
...
*/
   ...
   /*   
    QUnit.test( "Posting new Whiteboard with Blank Name", function( assert ) {
        ...
        TestUtils.sendPostRequest(Globals.baseURL + "rest/v1/collaboration/1002/whiteboard",data).then(function(result){
            assert.ok(result != null,"Response should not be null !");
    ...
    QUnit.test( "Posting new Whiteboard with invalid Collaboration Id", function( assert ) {
        ...
        TestUtils.sendPostRequest(Globals.baseURL + "rest/v1/collaboration/9999/whiteboard",data).then(function(result){
            assert.ok(result != null,"Response should not be null !");
