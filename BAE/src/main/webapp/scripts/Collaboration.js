(function( module, test ) {
    ...
    QUnit.test("Reading Collaborations of Neighborhood ID=1", function(assert){
        ...
        TestUtils.sendGetRequest( Globals.baseURL+"rest/v1/neighborhood/1/collaboration").then(function(result){
            if(result.length>0)
            {
                count_flag=true;
            }
    ...
    QUnit.test("Reading Collaborations of Neighborhood when collaboration are not exist", function(assert){
        ...
        TestUtils.sendGetRequest( Globals.baseURL+"rest/v1/neighborhood/30/collaboration").then(function(result){
            if(result.length == 0 )
            {
                count_flag=true;
            }                
