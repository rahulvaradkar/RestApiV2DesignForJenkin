...
module('Neighbourhood');
	...
	QUnit.test( "Reading Neighborhood with ID=1", function( assert ) {
		...
		TestUtils.sendGetRequest(Globals.baseURL + "rest/v1/neighborhood/1").then(function(result){
			var flag=false;
			if(result.length>0)
			{
				count_flag=true;
    ...
    QUnit.test( "Posting new Neighborhood at level 2", function( assert ) {
		...
		var data = {
		"level":"2",
		"name": "BoardwalkAPI_123",
		"id":"0",
		"parentId": "278",
		"secure": "true"
		}
		TestUtils.sendPostRequest(Globals.baseURL + "rest/v1/neighborhood",data).then(function(result){
			nhid_=result[0].id;
			console.log(nhid_);
      ...
      QUnit.test("Delete test case for neighborhood at level 2", function(assert){
        var done=assert.async();       
		TestUtils.sendDeleteRequest(Globals.baseURL + "rest/v1/neighborhood/" +nhid_).then(function(res){
	...
	QUnit.test( "Posting new Neighborhood at level 0", function( assert ) {
      ...
      QUnit.test("Delete test case for neighborhood", function(assert){
        var done=assert.async();		
		TestUtils.sendDeleteRequest(Globals.baseURL + "rest/v1/neighborhood/" + nhid).then(function(res){
			assert.equal(typeof res, "string", "response should be string !");
	        assert.notEqual(res.length, 0, "should not be null !");
			var getResponse = res;
			var checkSubString = getResponse.indexOf("Neighborhood Deleted Successfully");
