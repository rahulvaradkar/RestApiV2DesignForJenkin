(function( module, test ) {
    ...
    QUnit.test("Reading Relation of Neighborhood ID=1", function( assert ) {
        ...
        TestUtils.sendGetRequest(Globals.baseURL +  "rest/v1/neighborhood/1/relation").then(function(result){
			if(result.length > 0)
                {
                    count_flag=true;
                }
   ...
   /*  
    QUnit.test( "Creating Relation", function( assert ) {
        ...
        TestUtils.sendPostRequest(Globals.baseURL + "rest/v1/neighborhood/1/relation",data).then(function(result){
