...
String selectedRelation =(String) request.getAttribute("selectedRelation");
String relationName = " Enter Name here ";
...
else
{
  ...
  System.out.println("selNhid = " + nhId + " selectedRelation = " + selectedRelation + "  relationname = " + relationName );
request.setAttribute("heading", "Neighborhood Tree");
request.setAttribute("heading", "Select Neighborhoods for Relation");
...
Vector selectedNeighborhoods =null;

if ( selectedRelation != null &&  ! selectedRelation.trim().equals("") )
{
	selectedNeighborhoods = (Vector) relationshipToNeighborhoods.get(selectedRelation);
}
%>
...
<input type="hidden" name="selectedRelation" value="<%=selectedRelation%>">
...
<%
	...
	checked = selectedNeighborhoods.contains(new Integer(nhid));
...
<%
	 ...
	 // iterate over the neighborhood tree
		...
		if (selectedNeighborhoods != null )
		 checked = selectedNeighborhoods.contains(new Integer(nh1id));
...
<%
			...
			if (selectedNeighborhoods != null )
			 checked = selectedNeighborhoods.contains(new Integer(nh2id));
...
<%
			...
			while (nh3Iter.hasNext()) {
				...
				if (selectedNeighborhoods != null )
				 checked = selectedNeighborhoods.contains(new Integer(nh3id));
...
%>
	<%=selectedRelation%>
	<input type="hidden"  name="relName" value="<%=relationName%>">
<%
}
else
{
%>
	<input type=text name="relName" value="<%=relationName%>">
<%}%>
<br>
<br>
...
your site's HTML. -->
		...
		<tr>
		<td> <a href="javascript:saveRelation()">Save</a> </td>
		</tr>
		</td></tr></table></table>
		</tr></table>
