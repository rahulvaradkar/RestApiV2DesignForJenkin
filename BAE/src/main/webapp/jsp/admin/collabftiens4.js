...
// *****************************************************************
function Folder(folderDescription, hreference,checkDOMId) //constructor
{
  //constant data
...
function FolderCB(folderDescription, hreference,checkDOMId,checkVal,cBox, cBoxLink, selectCB) //constructor
{
  //constant data
  this.desc = folderDescription;
  this.hreference = hreference;
  ...
  this.parentObj = null;
  this.maySelect=true;
  //System.out.println("check DOM Id" + checkDOMId);
  this.prependHTML ="";

  if (cBox) {
        if (selectCB == true) {
      
  	 this.prependHTML = "<input " + "  type=checkbox id="
	  				+checkDOMId
					+ "  name="
					+checkDOMId
					+" value="
...
function drawFolder(insertAtObj)
{
  ...
  if (WRAPTEXT)
	  docW = docW + this.prependHTML+"<td valign=middle width=100%>"
  else
	  docW = docW + this.prependHTML+"<td valign=middle nowrap width=100%>"
...
function setStateFolder(isOpen)
{
  ...
  if (this.getID()!=collabTree.getID() && PERSERVESTATE && !this.isOpen) //closing
  {
...
function ItemCB(itemDescription, itemLink, target,checkDOMId,checkVal,cBox, cBoxLink ,selectCB) // Constructor
{
...
function clickOnNodeObj(folderObj)
{
  ...
  if (folderObj.id!=collabTree.id && PERSERVESTATE)
  {
    currentOpen = GetCookie("clickedFolder")
    if (currentOpen == null)
      currentOpen = ""
...
function findObj(id)
{
  var i=0;
  var nodeObj;
  if (typeof collabTree.xID != "undefined") {
...
function gFldCB(description, hreference,checkDOMId,checkValue,cBox,cboxLink,iscboxChecked)
{
  folder = new FolderCB(description, hreference,checkDOMId,checkValue,cBox,cboxLink,iscboxChecked);
  return folder;
}
...
function gLnkCB(optionFlags, description, linkData,checkDOMId,checkVal,cBox,cboxLink,iscboxChecked)
{
  var fullLink = "";
  var targetFlag = "";
  var target = "";
  ...
  linkItem = new ItemCB(description, protocol+linkData, target,checkDOMId,checkVal,cBox,cboxLink,iscboxChecked)
  return linkItem
}
...
function storeAllNodesInClickCookie(treeNodeObj)
{
  ...
  if (typeof treeNodeObj.setState != "undefined") //is folder
  {
    ...
    if (treeNodeObj.getID() != collabTree.getID())
...
function CookieBranding(name) {
  if (typeof collabTree.treeID != "undefined")
    return name+collabTree.treeID //needed for multi-tree sites. make sure treeId does not contain cookieCutter
  else
    return name
}
...
function initializeDocument()
{
  ...
  //collabTree (with the site's data) is created in an external .js (demoFramesetNode.js, for example)
  collabTree.initialize(0, true, "")
  if (supportsDeferral && !STARTALLOPEN)
	  collabTree.renderOb(null) //delay construction of nodes
  else {
    renderAllTree(collabTree, null);

    if (PERSERVESTATE && STARTALLOPEN)
      storeAllNodesInClickCookie(collabTree)
    ...
    if (browserVersion != 0 && !STARTALLOPEN)
      hideWholeTree(collabTree, false, 0)
  }
