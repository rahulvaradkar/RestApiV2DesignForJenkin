...
public class httpt_vb_login_test {
	@Mock
...
//		encode to Base64 string
		...
		when(request.getAttribute("userName")).thenReturn(ds.getUsername());
		when(request.getAttribute("templateMode")).thenReturn(ds.getTemplateMode());
		when(request.getSession(true)).thenReturn(session);
		when(response.getOutputStream()).thenReturn(servletOut);
		new httpt_vb_Login().service(request, response);
		...
		AssertJUnit.assertNotNull(res[0]);
