SELECT  BW_CELL.ID   CELL_ID, BW_CELL.CELL_TYPE,
	BW_STRING_VALUE.STRING_VALUE   CELL_STRING_VALUE, 	
	-1 CELL_INTEGER_VALUE,
	1.1 CELL_DOUBLE_VALUE,
	-1 CELL_TBL_VALUE,
	BW_STRING_VALUE.TX_ID,
	BW_TXS.CREATED_ON,
	BW_TXS.CREATED_BY,
	BW_TXS.DESCRIPTION,
	BW_TXS.COMMENT_,
	BW_USER.EMAIL_ADDRESS,
	BW_CELL.BW_ROW_ID   ROW_ID,
	BW_CELL.BW_COLUMN_ID   COLUMN_ID,
	BWCOLUMN.NAME   COLUMN_NAME,
	BWTABLE.NAME   TABLE_NAME
FROM 
(
SELECT BW_STRING_VALUE.BW_CELL_ID, MAX ( BW_STRING_VALUE.ID )    MAX_VALUE_ID
FROM BW_CELL, BW_ROW, BW_STRING_VALUE
WHERE BW_CELL.BW_ROW_ID = BW_ROW.ID
     AND BW_CELL.ID = ?
      AND BW_STRING_VALUE.TX_ID < BW_CELL.TX_ID
      AND BW_CELL.ID = BW_STRING_VALUE.BW_CELL_ID
      AND BW_ROW.IS_ACTIVE = 1             
GROUP BY BW_STRING_VALUE.BW_CELL_ID 
)
  MAXVALUETBL,
BW_CELL, BW_ROW, BW_STRING_VALUE,BW_TXS,BW_USER,BW_COLUMN    BWCOLUMN,BW_TBL   BWTABLE
WHERE BW_CELL.BW_ROW_ID = BW_ROW.ID
      AND BW_CELL.ID = ?
      AND  BW_CELL.BW_COLUMN_ID = BWCOLUMN.ID
      AND BW_CELL.ID = BW_STRING_VALUE.BW_CELL_ID
      AND BW_CELL.ID = MAXVALUETBL.BW_CELL_ID      
      AND BW_STRING_VALUE.ID = MAXVALUETBL.MAX_VALUE_ID
      AND BW_CELL.CELL_TYPE='STRING'
     AND  BW_STRING_VALUE.TX_ID = 	BW_TXS.TX_ID
     AND BW_USER.ID = BW_TXS.CREATED_BY
     AND  BW_ROW.BW_TBL_ID = BWTABLE.ID
 UNION ALL 
SELECT  BW_CELL.ID   CELL_ID, 
	BW_CELL.CELL_TYPE,
	' ' CELL_STRING_VALUE, 
	BW_INTEGER_VALUE.INTEGER_VALUE   CELL_INTEGER_VALUE,
	1.1 CELL_DOUBLE_VALUE,
	-1 CELL_TBL_VALUE,
	BW_INTEGER_VALUE.TX_ID,
	BW_TXS.CREATED_ON,
	BW_TXS.CREATED_BY,
	BW_TXS.DESCRIPTION,
	BW_TXS.COMMENT_,
	BW_USER.EMAIL_ADDRESS,
	BW_CELL.BW_ROW_ID   ROW_ID,
	BW_CELL.BW_COLUMN_ID   COLUMN_ID,
	BWCOLUMN.NAME   COLUMN_NAME,
	BWTABLE.NAME   TABLE_NAME
FROM 
(
SELECT BW_INTEGER_VALUE.BW_CELL_ID, MAX ( BW_INTEGER_VALUE.ID )   MAX_VALUE_ID
FROM BW_CELL, BW_ROW, BW_INTEGER_VALUE
WHERE BW_CELL.BW_ROW_ID = BW_ROW.ID
      AND BW_CELL.ID = ?
      AND BW_INTEGER_VALUE.TX_ID < BW_CELL.TX_ID
      AND BW_ROW.IS_ACTIVE = 1
      AND BW_CELL.ID = BW_INTEGER_VALUE.BW_CELL_ID      
GROUP BY BW_INTEGER_VALUE.BW_CELL_ID 
)
  MAXVALUETBL,
BW_CELL, BW_ROW, BW_INTEGER_VALUE, BW_TXS,BW_USER,BW_COLUMN    BWCOLUMN,BW_TBL   BWTABLE
WHERE BW_CELL.BW_ROW_ID = BW_ROW.ID
      AND BW_CELL.ID = ?
      AND  BW_CELL.BW_COLUMN_ID = BWCOLUMN.ID
      AND BW_CELL.ID = BW_INTEGER_VALUE.BW_CELL_ID
      AND BW_CELL.ID = MAXVALUETBL.BW_CELL_ID      
      AND BW_INTEGER_VALUE.ID = MAXVALUETBL.MAX_VALUE_ID
      AND BW_CELL.CELL_TYPE='INTEGER'
      AND  BW_INTEGER_VALUE.TX_ID = 	BW_TXS.TX_ID
     AND BW_USER.ID = BW_TXS.CREATED_BY	
  AND  BW_ROW.BW_TBL_ID = BWTABLE.ID
 UNION ALL 
SELECT  BW_CELL.ID   CELL_ID, BW_CELL.CELL_TYPE,
	' ' CELL_STRING_VALUE, 
	1 CELL_INTEGER_VALUE,
	BW_DOUBLE_VALUE.DOUBLE_VALUE    CELL_DOUBLE_VALUE,
	-1 CELL_TBL_VALUE,	
	BW_DOUBLE_VALUE.TX_ID,
	BW_TXS.CREATED_ON,
	BW_TXS.CREATED_BY,
	BW_TXS.DESCRIPTION,
	BW_TXS.COMMENT_,
	BW_USER.EMAIL_ADDRESS,
	BW_CELL.BW_ROW_ID   ROW_ID,
	BW_CELL.BW_COLUMN_ID   COLUMN_ID,
	BWCOLUMN.NAME   COLUMN_NAME,
	BWTABLE.NAME   TABLE_NAME
FROM 
(
SELECT BW_DOUBLE_VALUE.BW_CELL_ID, MAX ( BW_DOUBLE_VALUE.TX_ID )   MAX_VALUE_ID
FROM BW_CELL, BW_ROW, BW_DOUBLE_VALUE
WHERE BW_CELL.BW_ROW_ID = BW_ROW.ID
     AND BW_CELL.ID = ?
         AND BW_DOUBLE_VALUE.TX_ID < BW_CELL.TX_ID
      AND BW_ROW.IS_ACTIVE = 1
       AND BW_CELL.ID = BW_DOUBLE_VALUE.BW_CELL_ID       
GROUP BY BW_DOUBLE_VALUE.BW_CELL_ID 
)
  MAXVALUETBL,
BW_CELL, BW_ROW, BW_DOUBLE_VALUE, BW_TXS,BW_USER,BW_COLUMN    BWCOLUMN,BW_TBL   BWTABLE
WHERE BW_CELL.BW_ROW_ID = BW_ROW.ID
	AND BW_CELL.ID = ?
	AND  BW_CELL.BW_COLUMN_ID = BWCOLUMN.ID
	AND BW_CELL.ID = BW_DOUBLE_VALUE.BW_CELL_ID
	AND BW_CELL.ID = MAXVALUETBL.BW_CELL_ID      
	AND BW_DOUBLE_VALUE.ID = MAXVALUETBL.MAX_VALUE_ID
	AND BW_CELL.CELL_TYPE='FLOAT'
	AND  BW_DOUBLE_VALUE.TX_ID = 	BW_TXS.TX_ID
	AND BW_USER.ID = BW_TXS.CREATED_BY	
	AND  BW_ROW.BW_TBL_ID = BWTABLE.ID