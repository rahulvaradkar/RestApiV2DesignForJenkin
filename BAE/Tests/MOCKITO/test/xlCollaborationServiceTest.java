...
import com.boardwalk.excel.xlError;
...
public class xlCollaborationServiceTest extends xlService
implements SingleThreadModel
{
	...
	throws ServletException, IOException
    {
        ...
        if ( loginUser() )
        {
			...
			action = 10;//Integer.parseInt(wrkstr);
			try
			{
				if (action == 1)
				{
...
//				}
				...
				System.out.println("the response buffer is --->"+ responseBuffer);
			}
...
public String addNewTableForCollabName()
	...
	// Call
		...
		try
		{
			...
			//DatabaseLoader databaseloader = new DatabaseLoader(new Properties());
			connection = xlLinkExportServiceTest.getConnectionForTest();//databaseloader.getConnection();

			String export_location = ds.getExportDataLocation();
			String wrkstr;
			//wrkstr = st.nextToken (Seperator);
			collabName =ds.getCollabName();//wrkstr;
			...
			//wrkstr = st.nextToken (Seperator);
			WhiteboardName =  ds.getWbName();//wrkstr;
			...
			if (collabId != -1 && wbId != -1)
			{
				//wrkstr = st.nextToken(Seperator);
				//st2 = new StringTokenizer(wrkstr);
				tableName = "TestTAS1"+ds.getTno();//st2.nextToken(ContentDelimeter);
...
//				}
				...
				String table=Integer.toString(a_table_id);
				int t_no=Integer.parseInt(tno);
				t_no++;
				//props.setProperty("Tablename", value);
				Properties props=new Properties();				
				FileOutputStream out = new FileOutputStream(export_location);
				FileInputStream in = new FileInputStream(export_location);
				props.load(in);
				props.setProperty("tno", Integer.toString(t_no));
				props.setProperty("LETableId", table);
...
//*/
    ...
    public boolean  loginUser()
	{
		...
		String wrkstr;
		//wrkstr = st.nextToken (Seperator);
		userId = 1000;//Integer.parseInt(wrkstr);
		System.out.println("User id :"+userId);
		
		//wrkstr = st.nextToken (Seperator);
		userName = "user1";//wrkstr;
		System.out.println("User Name :"+userName);
		//wrkstr = st.nextToken (Seperator);
		//userPassword = wrkstr;

		//wrkstr = st.nextToken (Seperator);
		memberId = 1000;//Integer.parseInt(wrkstr);
		System.out.println("Member id :"+memberId);
		
		//wrkstr = st.nextToken (Seperator);
		nhId = 2;//Integer.parseInt(wrkstr);
